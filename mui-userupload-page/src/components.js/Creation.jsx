import { CardContent, List, Card, Box, Grid, Stack,  Typography, Avatar, ListItem } from '@mui/material'
import React from 'react'

const Creation = () => {
  return (
    <Card sx={{ bgcolor: "#000", color: "#fff" }}>
      <CardContent>
        <Typography
          variant="h4"
          component="h1"
          sx={{
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            marginLeft: { xs: 3  },
            align: "center",
            marginTop: 5,
          }}
        >
          You are not alone . The pain of website creation is real
        </Typography>
      </CardContent>

      <Grid>
        <Grid
          container
          sx={{
            display: "flex",
            gap: { xs: 4 , sm: 2, md: 2 },
            alignItems: "center",
            justifyContent: "center",
            align: "center",
            marginTop: 3,
          }}
        >
          <Box sx={{ bgcolor: "#fff" }}>
            <Avatar
              alt="Travis Howard"
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRtQcud6X4ZQRXJUPR7wDtj96Cq82LBTc80lqHykkLExI7fqmbtocrgfgAxJfCs0RSIFBM&usqp=CAU"
            />
            <Typography align="center" sx={{ color: "#000" }}>
              "The easiest and most accuate way <br />
              to track time across teams and finish <br />
              the projects . Timely's automatic"
            </Typography>
            <br />
            <Typography variant="h6" sx={{ color: "#000" }}>
              Jerome Bell <br />{" "}
              <Stack sx={{ fontSize: 16, color: "gray" }}>CTO, Waverio</Stack>
            </Typography>
          </Box>

          <Box sx={{ bgcolor: "#fff" }}>
            <Avatar
              alt="Travis Howard"
              src="data:image/png;base64,"
            />
            <Typography align="center" sx={{ color: "#000" }}>
              "The easiest and most accuate way <br />
              to track time across teams and finish <br />
              the projects . Timely's automatic"
            </Typography>
            <br />
            <Typography variant="h6" sx={{ color: "#000" }}>
              Lesile Alexander <br />{" "}
              <Stack sx={{ fontSize: 16, color: "gray" }}>
                Precident of Marketing
              </Stack>
            </Typography>
          </Box>

          <Box sx={{ bgcolor: "#fff" }}>
            <Avatar
              alt="Travis Howard"
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQCU_MYMWarhwp_FhzqOiO78TqcU7u2yQc3xO5buehk5zDX7OTKtxZ5JV9VvYa7xSdM8Ho&usqp=CAU"
            />
            <Typography align="center" sx={{ color: "#000" }}>
              "The easiest and most accuate way <br />
              to track time across teams and finish <br />
              the projects . Timely's automatic"
            </Typography>
            <br />
            <Typography variant="h6" sx={{ color: "#000" }}>
              Wade Warren <br />{" "}
              <Stack sx={{ fontSize: 16, color: "gray" }}>Founder</Stack>
            </Typography>
          </Box>
        </Grid>
      </Grid>

      <CardContent
        align="center"
        sx={{ alignItems: "center", justifyContent: "center", marginTop: 4 }}
      >
        <Typography sx={{ mb: 2.5 }}>
          Trusted by world class business Owner's
        </Typography>
        <List
          sx={{
            display: "flex",
            fontSize: { xs: 12 , sm: 18 , md: 18 },
            fontWeight: { xs: 500 , sm: 500 , md: 500 },
            alignItems: "center",
            justifyContent: { xs: 'center' , sm: 'space-between' , md: 'space-between'},
            align: "center",
          }}
        >
          <ListItem>#PAPERZ</ListItem>
          <ListItem>@Dorfus</ListItem>
          <ListItem>#Martino</ListItem>
          <ListItem>[]SQUARE</ListItem>
          <ListItem>#GOBONA</ListItem>
        </List>
      </CardContent>
    </Card>
  );
}

export default Creation